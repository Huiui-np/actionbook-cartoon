...
//     This validates that the persistent CDP connection supports concurrent
//     operations on different tabs within the same session.
#[test]
fn tab_concurrent_multi_tab_same_session() {
    ...
    // (new-tab already waits 2s for page load internally)
    // Use '1+1' instead of document.title for reliability (no DOM dependency)
    let sid1 = sid.clone();
    let sid2 = sid.clone();
    let sid3 = sid.clone();
